<?php
/*
Plugin Name: VestaCP Dashboard Widget
Plugin URI: https://ss88.uk/
Description: Adds a widget to the Dashboard showing your VestaCP accounts details. Requires an API key from VestaCP.
Version: 1.0
...
function ss88_show_vestacp_widget() {
	...
	wp_enqueue_style('ss88_vestacp_widget_css', plugins_url('css.css', __FILE__));
	wp_enqueue_script('ss88_vestacp_widget_js', plugins_url('js.js', __FILE__));
...
function ss88_vestacp_widget_api($Vesta)
{
	...
	$answer = wp_remote_post($Vesta['URL'] . '/api/', array('body'=>$postvars));
...
function ss88_vestacp_widget_showbars($Data, $Vesta)
{
	...
	$DiskPercent = ($Data['DISK_QUOTA']=='unlimited') ? 'U' : $DiskPercent;
